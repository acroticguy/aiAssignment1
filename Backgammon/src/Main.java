                    ...
                    if(dice.isDouble()) {
                        ...
                        for (int i =1; i < 5; i++) {
                            if (board.isGraveyard()) { //Do we have pieces in our grave?
                                ...
                                user_input = myScanner.nextLine();  // Read user input
                                int starting_move = Integer.parseInt(user_input) + 1; // 1st space = 2nd element of the gameBoard Array, etc
                        ...
                        if (dice_selected == 1) {
                            for (int i = 0; i < 2; i++) {
                                if (board.isGraveyard()) {
                                    System.out.println("You have pieces in the Graveyard! You have to get them out before making any other move.");
                                    if (!board.isValidMove(1, 1 + dice.getDice()[i])) {
                                        System.out.println("This dice is not a valid move! Dice skipped.");
                                    } else {
                                        System.out.println("Placed a piece on position " + dice.getDice()[0] + ".");
                                        current_move = new Move(board.getLastPlayer(), 1, 1 + dice.getDice()[i] * board.getLastPlayer(), board.evaluate());
